...
//insertamos el afiliado  con la variable $parenid como padre posicional

}elseif($level2 < $ancho1){
    

$concr = $conectar->_db->query("SELECT ID FROM security_users WHERE parent_id = '$parentid' ORDER BY ID asc");
$suma1 = 0;
while($con = mysqli_fetch_array($concr)){
 
$qerrr = $conectar->_db->query("SELECT count(t2.ID) cont
FROM security_users AS t1
LEFT JOIN security_users AS t2 ON t2.parent_id=t1.ID
WHERE t1.ID='$con[ID]'");
$asd = mysqli_fetch_array($qerrr);
$conteo2 = $asd[cont];
if($conteo2 < $ancho){
    
    
     $queryrr = $conectar->_db->query("SELECT ID FROM security_users WHERE parent_id = '$con[ID]'");
$row_cntrr = $queryrr->num_rows;
    if($row_cntrr  >= '4'){
//el padre es el
    ...
    //el padre es su padre
    $queryrrt = $conectar->_db->query("SELECT preal FROM security_users WHERE ID = '$con[ID]'");
    $qq = mysqli_fetch_array($queryrrt);

    $real = $qq[0];
    
    }

$sql8 = $conectar->_db->query("INSERT INTO `security_users`(`identy_user`, `username`, `name_user`, `ape_user`,sex_user, dir_user,tel_user,pais_user,dpto_user	,ciu_user, `user_login`, `user_psw`, `parent_id`, `Id_Parent`, `creator`, `Nivel`, `activo_tree`, `coloca`,`user_register`, `user_rol`,bill,pack,`limit`,estado) VALUES ('$cedula_inscrito','$username','$nombre','$ape','$gene','$dir','$cel','$pais1','$departamento','$ciudad','$email','$pass1','$con[ID]','$parentid','$parentid','$nivelac','1','I',NOW(),'2','$bill','$pakis','$nuevafecha','1')");
...
$envia='Social Trading System'; 
$remite='info@https://socialtradingsystem.com'; 
...
//Fin envio de correos
 ...
 //echo "insert--> el padre qe se inserta es:-->".$con[id]."<br>";
...
$adse = $conectar->_db->query("SELECT lft,rgt FROM security_users WHERE parent_id = '$parentid' ORDER BY ID asc");
$sft = mysqli_fetch_array($adse);
$lft = $sft['lft'];
$rgt = $sft['rgt']; 
$concr =  $conectar->_db->query("SELECT ID FROM security_users WHERE  lft BETWEEN '$lft' and '$rgt'  ORDER BY ID asc");
$suma1 = 0;
while($con = mysqli_fetch_array($concr)){
...
$sql8 = $conectar->_db->query("INSERT INTO `security_users`(`identy_user`, `username`, `name_user`, `ape_user`,sex_user, dir_user,tel_user,pais_user,dpto_user	,ciu_user, `user_login`, `user_psw`, `parent_id`, `Id_Parent`, `creator`, `Nivel`, `activo_tree`, `coloca`,`user_register`, `user_rol`,bill,pack,`limit`,estado) VALUES ('$cedula_inscrito','$username','$nombre','$ape','$gene','$dir','$cel','$pais1','$departamento','$ciudad','$email','$pass1','$con[ID]','$parentid','$creator','$nivelac','1','I',NOW(),'2','$bill','$pakis','$nuevafecha','1')");
