...
class security_users {
    ...
    function consultar($id = 0) {
        if ($id != 0) {
            $sql = "SELECT * FROM security_users WHERE ID = " . $id;
            ...
            if (is_array($consulta) && !empty($consulta)) {
                ...
                $this->name_wll = $consulta['name_wll'];
...
//                $this->date_califica = $consulta['date_califica'];
                $this->token = $consulta['token'];
                $this->fec_expira_token = $consulta['fec_expira_token'];
            }
        }
    }
